    ...
    let card = `
        <div class="card">
            ...
            <p>Color: ${hoodie.color}</p>
             <p>${hoodie.description} <a href="item.html" onclick="addToSessionStorageHoodie(${hoodie.id})">Read more</a> </p>
            <p class="price">${hoodie.price} </p>
            <p><button onclick="addToCartHoodies(${hoodie.id})">Buy</button></p>
        </div>
    `;
...
tshirts.forEach(function (tshirt) {
    let card = `
        <div class="card">
            ...
            <p>${tshirt.description} <a href="item.html" onclick="addToSessionStorageTshirt(${tshirt.id})">Read more</a> </p>
            <p class="price">${tshirt.price}</p>
            <p><button onclick="addToCartTshirts(${tshirt.id})">Buy</button></p>
        </div>
    `;
...
jumpers.forEach(function (jumper) {
    let card = `
        <div class="card">
            ...
            <p>${jumper.description} <a href="item.html" onclick="addToSessionStorageJumper(${jumper.id})">Read more</a> </p>
            <p class="price">${jumper.price}  </p>
            <p><button onclick="addToCartJumpers(${jumper.id})">Buy</button></p>
        </div>
    `;
...
//Function to retrieve the data stored in the Session storage and display it in "item.html"
function viewProductDetails() {
    const productDetails = JSON.parse(sessionStorage.getItem("item"));


    let cardDetails = `
        <div class="card">
            <img class="item-image" src="${productDetails.image}">
            <h1 class="product-type">${productDetails.type}</h1>
    ...
    //displaying the card in the page
    document.getElementById("item-container").innerHTML = cardDetails;
}


//Adding the products to local storage and displaying a message indicating that the product has been added to "cart"
function addToCartHoodies(id) {
    const hoodie = hoodies[id];
    if(hoodie) {
...
function addToCartJumpers(id) {
    const jumper = tshirts[id];
    if(jumper) {
        const items = JSON.parse(localStorage.getItem('item') || "[]");
        items.push(jumper);
        localStorage.setItem('item', JSON.stringify(items));
...
//Displaying the items from local storage in the cart
function displayProductsInCart() {
    const products = JSON.parse(localStorage.getItem("item") || "[]");
    document.getElementById("cart-container").innerHTML = products.reduce((cards, product) =>
